...
# 问题长度,答案长度
buckets = [
    (5, 15),  # 问题长度最长为5， 回答长度最长为15
    (10, 20),  # 问题长度最长为10， 回答长度最长为20
    (15, 25),  # 问题长度最长为15， 回答长度最长为25
    (20, 30)  # 问题长度最长为20， 回答长度最长为30
]
...
def sentence_indice(sentence):
    """
    将传入的文本sentence转换为数字id，如果字符未知的，那么将其转换为UNK对应的id
    :param sentence:
    :return:
    """
    ret = []
...
def generate_dictionary(input_dir, directory_path='dictionary_detail.json'):
    # 从input_dir列出数据库列表
    db_paths = []
    ...
    for db_path in db_paths:
        ...
        with open(directory_path, 'w', encoding='utf-8') as fp:
        ...
        def _insert(wait_insert):
            if len(wait_insert) > 0:
                for encoder_size, decoder_size, ask, answer in wait_insert:
                    ...
                    cur.execute(
                        "INSERT INTO conversation (ask, answer) VALUES ('{}', '{}');".format(ask.replace("'", "''"),
                                                                                             answer.replace("'", "''")))
                    all_inserted[key] += 1
...
def fetch_data_to_file():
    with open('./db/conversation.txt', 'w', encoding='utf-8') as writer:
        split_char = '\t'
        ...
        ret = c.execute('''SELECT ask, answer FROM conversation;''')
        for ask, answer in tqdm(ret, total=total):
            ask = ask.replace(split_char, ' ').strip()
            answer = answer.replace(split_char, ' ').strip()
            writer.writelines("{}{}{}\n".format(ask, split_char, answer))
        print(total)


if __name__ == '__main__':
    # print('generate director dbs')
    # generate_dictionary('./db')
