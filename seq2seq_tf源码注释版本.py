...
def _extract_argmax_and_embed(embedding,
    ...
    def loop_function(prev, _):
        # prev表示上一个时刻RNN Cell的输出
        if output_projection is not None:
            # 如果给定投影参数，做线性转换；如果没有给定的时候，实际上prev就是投影之后的值(core_rnn_cell.OutputProjectionWrapper做了一个转换)
            # 结果可以认为是：[batch_size, vocab_size] --> 每个样本属于各个词汇的可能性概率矩阵
            prev = nn_ops.xw_plus_b(prev, output_projection[0], output_projection[1])
        # 选择概率最大的那个词汇作为当前样本上一个时刻的预测值(得到的是id)
        prev_symbol = math_ops.argmax(prev, 1)
        # Note that gradients will not propagate through the second parameter of
        # embedding_lookup.
        # 基于id获取当前id对应的embedding vector向量，将这个向量返回作为下一个时刻的输入
        emb_prev = embedding_ops.embedding_lookup(embedding, prev_symbol)
        if not update_embedding:
            # 停止该操作对于Embedding的梯度的计算(在反向传播的过程中，不会计算这个路径对于embedding的梯度更新的影响)
...
def attention_decoder(decoder_inputs,  # embedding之后的输入值
                      initial_state,
                      attention_states,
                      cell,
                      output_size=None,
    ...
    with variable_scope.variable_scope(
        ...
        attn_size = attention_states.get_shape()[2].value  # 获取Attention的向量维度大小，也就是Encoder RNN Cell的输出维度大小
        ...
        hidden_features = []
        v = []
        attention_vec_size = attn_size  # 将Attention计算过程中，编码器输入状态映射的维度大小设置为和输入维度大小一致
        ...
        def attention(query):
            ...
            for a in xrange(num_heads):
                with variable_scope.variable_scope("Attention_%d" % a):
                    y = Linear(query, attention_vec_size, True)(query)  # [None, attention_vec_size]
                    y = array_ops.reshape(y, [-1, 1, 1, attention_vec_size])  # [None, 1, 1, attention_vec_size]
                    # Attention mask is a softmax of v^T * tanh(...).
                    # [attention_vec_size] * [None, encoder_sequence_length, 1, attention_vec_size]
                    # --> [None, encoder_sequence_length, 1, attention_vec_size]
                    # --> [None, encoder_sequence_length]
                    s = math_ops.reduce_sum(v[a] * math_ops.tanh(hidden_features[a] + y), [2, 3])
                    a = nn_ops.softmax(s)  # [None, encoder_sequence_length]
                    # Now calculate the attention-weighted vector d.
                    # [None, encoder_sequence_length] -> [None, encoder_sequence_length, 1, 1]
                    # [None, encoder_sequence_length, 1, 1] * [None, encoder_sequence_length, 1, num_units]
                    # --> [None, num_units]
        ...
        outputs = []  # 解码器最终输出值保存的列表集合
        prev = None  # 保存的解码器上一个时刻的输出值(对于第一个时刻而言，prev为None)
        batch_attn_size = array_ops.stack([batch_size, attn_size])  # 构建一个Shape对象，值为:[batch_size, atten_size]
        attns = [  # 定义的Attention output的值，初始为0., 形状为: [batch_size, atten_size], 也就是当前批次中的每个样本一个attention的输出向量
            array_ops.zeros(
                batch_attn_size, dtype=dtype) for _ in xrange(num_heads)
        ]
        for a in attns:  # Ensure the second shape of attention vectors is set.
            a.set_shape([None, attn_size])
        if initial_state_attention:  # 当为True的时候，表示对第一个时刻的State也计算Attention的值，默认不计算，使用0
            attns = attention(
                initial_state)  # 当参数initial_state_attention设置为True的时候，对编码器输出状态/解码器的初始状态进行Attention转换操作；默认是不进行。
        for i, inp in enumerate(decoder_inputs):
            if i > 0:
                variable_scope.get_variable_scope().reuse_variables()  # 除了第一个时刻进行参数初始化外，其它所有时刻，参数全部共享
            # If loop_function is set, we use it instead of decoder_inputs.
            if loop_function is not None and prev is not None:  # 测试的时候会将上一个时刻的预测值当做当前时刻的输入
                with variable_scope.variable_scope("loop_function", reuse=True):
                    inp = loop_function(prev, i)
            # Merge input and previous attentions into one vector of the right size.
            input_size = inp.get_shape().with_rank(2)[1]  # 得到当前解码器输入的样本维度大小
            if input_size.value is None:
                raise ValueError("Could not infer input size from input: %s" % inp.name)

            inputs = [inp] + attns  # 将当前时刻解码器输入yt以及上一个时刻对应的Attention output(ct，ct的计算是基于时刻t的State来计算)合并
            x = Linear(inputs, input_size, True)(inputs)  # 在输入到RNN Cell之前，对输入和Attention合并的值做一个全连接转换，输出维度为:y的维度大小
            # Run the RNN. 给定当前输入<包含当前时刻的解码器输入和上一个时刻的Attention状态值>以及上一个时刻的状态，得到当前的输出以及当前时刻的状态
            cell_output, state = cell(x, state)
            # Run the attention mechanism. # 基于当前时刻的解码器状态计算Attention的值
            if i == 0 and initial_state_attention:
                with variable_scope.variable_scope(
                        variable_scope.get_variable_scope(), reuse=True):
                    attns = attention(state)
            else:
                # 基于当前时刻的状态产生Attention的值(当前时刻的Attention Output)，主要用于下一个时刻的输入以及当前时刻的输出
                attns = attention(state)

            # 将当前时刻的Attention值和当前时刻Decoder RNN Cell的输出值合并做线性转换，将转换后的值作为解码器的最终输出
            with variable_scope.variable_scope("AttnOutputProjection"):
                inputs = [cell_output] + attns  # 合并当前时刻的Cell输出和基于当前时刻State计算出来的Attention值
                output = Linear(inputs, output_size, True)(inputs)  # 做线性转换，输出值作为RNN Cell的输出

            # 如果给定loop_function函数，表示在下一个时刻使用当前时刻的输出值作为输入值，所以将当前时刻的输出赋值为prev变量
            if loop_function is not None:
                prev = output  # 只用于预测解码阶段
            outputs.append(output)
...
def embedding_attention_decoder(decoder_inputs,
    ...
    with variable_scope.variable_scope(
        ...
        # 这里之所以要有一个embedding table，是应该解码器和编码器数据可能是不一致的，所以需要单独的进行embedding转换操作
        embedding = variable_scope.get_variable("embedding",
                                                [num_symbols, embedding_size])
        # 根据参数feed_previous值，返回对应的信息；
        # 当参数feed_previous为True的时候，返回一个函数，在这个函数中利用embedding和output_projection以及RNN Cell的当前时刻的输出，做投影转换得到预测的值对应的词向量，并返回
        loop_function = _extract_argmax_and_embed(
            embedding, output_projection,
            update_embedding_for_previous) if feed_previous else None
        # 对输入的Decoder的值进行Embedding提取转换操作
        emb_inp = [
            embedding_ops.embedding_lookup(embedding, i) for i in decoder_inputs
        ]
        return attention_decoder(
            emb_inp,  # decoder的时候输入值(Embedding之后的)
            initial_state,  # decoder的初始化状态值， 也就是encoder编码器的最终状态值
            attention_states,  # 计算Attention的时候，使用到的encoder输出值组成的一个tensor对象；[batch_size, time_steps, output_dimensions]
            cell,  # 基础的RNN cell，也就是decoder rnn cell
...
def embedding_attention_seq2seq(encoder_inputs,
    ...
    with variable_scope.variable_scope(
        ...
        dtype = scope.dtype
        # Encoder. ： 编码过程: Embedding lookup --> static rnn --> output+state
        encoder_cell = copy.deepcopy(cell)
        encoder_cell = core_rnn_cell.EmbeddingWrapper(
            encoder_cell,
            embedding_classes=num_encoder_symbols,  # vocab size
            embedding_size=embedding_size)  # embedding vector size
        ...
        attention_states = array_ops.concat(top_states, 1)  # [batch_size, sequence_length, output_size/hidden_units]
        ...
        if isinstance(feed_previous, bool):
            return embedding_attention_decoder(
                decoder_inputs,  # 外部传入的Decoder输入Tensor列表
                encoder_state,  # 编码器的最终输出状态, 作为解码器的初始状态C
                attention_states,  # 编码器最终输出的输出值，concat合并后作为attention的states的值(用于计算Attention的值)
                cell,  # 基础RNN Cell，也就是解码器的Cell
                num_decoder_symbols,  # 类别数目(解码输出可能的类别数目，词汇表大小)
                embedding_size,  # Embedding size(词向量维度大小)
                num_heads=num_heads,  # 外部传入，在我们的聊天机器人中默认为1<参考了Transformer的结构，叫做多头Attention机制>
                output_size=output_size,
                output_projection=output_projection,  # 解码输出的参数列表
                feed_previous=feed_previous,
                initial_state_attention=initial_state_attention  # 对于解码器的第一个时刻而言，是否计算Attention值，默认不计算
            )
...
def sequence_loss_by_example(logits,
    ...
    with ops.name_scope(name, "sequence_loss_by_example",
        ...
        log_perp_list = []
        for logit, target, weight in zip(logits, targets, weights): # 对每个时刻的输出计算损失函数
    ...
    with ops.name_scope(name, "model_with_buckets", all_inputs):
        for j, bucket in enumerate(buckets):
            with variable_scope.variable_scope(
                ...
                if per_example_loss:  # True表示每个样本一个损失，False表示所有样本的损失求均值(默认)
                    losses.append(
                        sequence_loss_by_example(
                            outputs[-1],
                            targets[:bucket[1]],
                    ...
                    losses.append(
                        sequence_loss(
                            outputs[-1],  # 当前桶对应的输出值，也就是解码器输出
                            targets[:bucket[1]],  # 当前桶对应的实际输出值
                            weights[:bucket[1]],  # 当前桶对应的weights，也就是在没有填充的位置为1，填充位置为1的这样的一个mask的输入值
                            softmax_loss_function=softmax_loss_function))
